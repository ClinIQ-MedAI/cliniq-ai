...
# Shared imaging utilities (DICOM decoding) live at the repo root.
try:
    import sys as _sys
    _CLINIQ_ROOT = str(Path(__file__).resolve().parents[1])
    ...
    DICOM_SUPPORT = True
except Exception as _dicom_exc:  # noqa: BLE001
    DICOM_SUPPORT = False
    print(f"⚠️ DICOM support disabled: {_dicom_exc}")

# RAG grounding (optional): ground health answers in the curated knowledge base.
# Enabled by default when an index exists; set RAG_ENABLED=0 to disable.
RAG_SUPPORT = False
if os.getenv("RAG_ENABLED", "1") != "0":
    try:
        _cr = str(Path(__file__).resolve().parents[1])
        if _cr not in __import__("sys").path:
            __import__("sys").path.insert(0, _cr)
        from rag import grounding_block as _rag_grounding, sources as _rag_sources
        RAG_SUPPORT = True
    except Exception as _rag_exc:  # noqa: BLE001
        print(f"⚠️ RAG grounding disabled: {_rag_exc}")


def _screen_upload(image_bytes, image_type):
    """
    Run the OOD/quality gate on an upload. Returns a GateResult or None (if the
    imaging package is unavailable or the gate errors — never blocks on failure).
    'dental' in the chatbot means a panoramic X-ray -> map to the grayscale gate.
    """
