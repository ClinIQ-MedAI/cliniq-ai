...
# Consumes `cliniq:jobs:dental_xray` and publishes to `cliniq:results` when
...
try:
    ...
    install_request_logging(app, "oral-xray")
except Exception as _log_exc:  # noqa: BLE001 - logging must never crash a service
    print(f"[oral-xray] request logging not installed: {_log_exc}")
try:
    from messaging.fastapi_integration import attach_worker

    attach_worker(app, modality="dental_xray", route=predict_for_llm)
except Exception as _queue_exc:  # noqa: BLE001
    print(f"[oral-xray] queue worker not attached: {_queue_exc}")
