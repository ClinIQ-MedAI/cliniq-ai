...
# --- CONFIGURATION ---
DATA_YAML = "/N/u/moabouag/Quartz/Documents/cliniq/wrest/data/graz/data.yaml"
PROJECT_DIR = "/N/scratch/moabouag/grazpedwri/runs/train"
MODEL_NAME = "yolov8x_graz_896_xray_v2"
...
def main():
    ...
    # Resume from best checkpoint (avoids NaN-corrupted last.pt)
    checkpoint_path = os.path.join(PROJECT_DIR, MODEL_NAME, "weights", "best.pt")
    ...
    if os.path.exists(checkpoint_path):
        ...
        print("   Starting fresh from yolov8x.pt")
    ...
    # Train with optimized X-ray settings
    results = model.train(
        data=DATA_YAML,
        epochs=100,             # Increased for better convergence
        imgsz=896,              # Good balance for X-ray detail
        batch=4,                # Reduced to prevent OOM
        workers=0,              # Zero workers to reduce RAM usage
        project=PROJECT_DIR,
        name=MODEL_NAME,
        exist_ok=True,
        resume=False,           # Start fresh from best.pt
        ...
        lr0=0.0005,             # Much lower LR to prevent gradient explosion
        lrf=0.01,               # Final LR = lr0 * lrf (cosine decay)
        warmup_epochs=3,        # Warmup to stabilize training
        
        # Optimizer
        optimizer='AdamW',      # Better for medical imaging
        weight_decay=0.0005,
        
        # Safe X-ray Augmentations
        augment=True,
        mosaic=0.3,             # Moderate mosaic (helps diversity)
        mixup=0.1,              # Low mixup
        close_mosaic=10,        # Disable mosaic in last 10 epochs for accuracy
        fliplr=0.5,             # Horizontal flip OK
        flipud=0.0,             # No vertical flip for X-ray
        degrees=5.0,            # Very slight rotation
        translate=0.1,          # Slight translation
        scale=0.2,              # Slight scale jitter
        hsv_v=0.2,              # Slight brightness variation only
        ...
        device=device,
        verbose=True,
        amp=False,              # Disable AMP to prevent NaN (V100 fix)
